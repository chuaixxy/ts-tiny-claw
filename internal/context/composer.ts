// internal/context/composer.ts
// 对应 Go: internal/context/composer.go
//
// 将基础内核身份、AGENTS.md 和 SkillLoader 解析出的 Skills 动态拼接成最终的系统级提示词。

import { existsSync, readFileSync } from "node:fs"
import path from "node:path"

import type { Message } from "../schema/message.ts"
import { createSkillLoader, type SkillLoader } from "./skill.ts"

/** PromptComposer 负责根据工作区环境动态生成 System Prompt */
export class PromptComposer {
  private readonly workDir: string
  private readonly planMode: boolean // 【新增】计划模式开关
  private readonly skillLoader: SkillLoader

  constructor(workDir: string, planMode = false) {
    this.workDir = workDir
    this.planMode = planMode
    // 对应 Go: skillLoader: NewSkillLoader(workDir)
    this.skillLoader = createSkillLoader(workDir)
  }
...
`)
    ...
    if (existsSync(agentsMDPath)) {
      try {
        ...
        // 对应 Go: if err == nil { ... } —— 读失败则跳过 AGENTS.md
      }
    }
    ...
    // 对应 Go: return schema.Message{ Role: schema.RoleSystem, Content: ... }
    return {
      role: "system",
      content: parts.join(""),
    }
...
/** 工厂：对应 Go 的 NewPromptComposer */
export function createPromptComposer(
  workDir: string,
  planMode = false,
): PromptComposer {
  return new PromptComposer(workDir, planMode)
}
